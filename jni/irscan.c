...
Java_com_example_hellojni_HelloJni_open
 (JNIEnv* env,jobject thiz,jstring path, jint baudrate) {
	  	...
	  	if (set_Parity(openfd,8,1,'N')== CONFIG_DEV_ERROR)
  		{
   		...
   		//open irdev
   		fd1 = open("/dev/irdev",O_RDWR);
		if(fd1<0)
		{
			LOGE("open irdev error\n");
			return CONFIG_DEV_ERROR;
		}
		irdev = fd1;
		LOGE("open irdve ok fd1=%d", fd1);
...
static int   ir_trig(int fd)
{
	int ret;

	ret =ioctl(fd, IO_CTL_TRIG_L,NULL);
	if(ret<0)
	{
	...
	usleep(Scan_Time);  //10ms

	ret = ioctl(fd, IO_CTL_TRIG_H,NULL);
	if(ret<0)
	{
		LOGE("IO_CTL_TRIG_H error\n");
		return -1;
	}
...
jobject
Java_com_example_hellojni_HelloJni_readirscan(JNIEnv* env, jobject thiz,  jint timeout, jint scan_interval){
	char buf[1024] = {0};
	int num, flage;
	...
	while(1){
		...
		//for(i = 0; i < strlen; i++)
			//LOGE("read data %x", buf[i]);
		return (*env)->NewStringUTF(env, buf);//返回得到的字符串数据
}
